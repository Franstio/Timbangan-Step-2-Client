...
const Home = () => {
  ...
  const sendLockBottom = async () => {
    try {
      const response = await apiClient.post(
        `http://${bottomLockHostData.hostname}.local:5000/lockBottom`,
        {
          idLockBottom: 1,
        }
      );
  ...
  const sendGreenlampOnCollection = async () => {
    try {
      const response = await apiClient.post(
        `http://${bottomLockHostData.hostname}.local:5000/greenlampon`,
        {
          idLampGreen: 1,
        }
      );
  ...
  const sendYellowOffCollection = async () => {
    try {
      const response = await apiClient.post(
        `http://${bottomLockHostData.hostname}.local:5000/yellowlampoff`,
        {
          idLampYellow: 1,
        }
      );
  ...
  useEffect(() => {
    if (
      bottomLockHostData.binId &&
      bottomLockHostData.hostname &&
      bottomLockHostData.binId != "" &&
      bottomLockHostData.hostname != ""
    ) {
      new Promise(async () => {
